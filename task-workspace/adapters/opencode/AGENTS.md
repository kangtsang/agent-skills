...
- `--share-memory` — no-op for opencode: instructions/rules live in the repo's
  `AGENTS.md` / `CLAUDE.md`, which travel with the worktree, so memory is
  already shared. Omit it.

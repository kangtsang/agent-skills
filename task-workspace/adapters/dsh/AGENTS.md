...
- `--share-memory` — no-op for dsh: DSH loads the instruction-file chain
  (`$DSH_HOME/AGENTS.md` + per-directory `AGENTS.md`/`CLAUDE.md`), which travels
  with the worktree, so memory is already shared. Omit it.
